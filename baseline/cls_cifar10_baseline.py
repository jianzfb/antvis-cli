...
project = 'AA'
experiment = 'ShuffleNetV2'

# 创建实验
mlogger.config(project, experiment, token='8f8014f0434a426491d20726261e7ccd', server='BASELINE')
...
top_1=[]
top_5=[]
with open("./imagenet-cls/ShuffleNetV2.log", 'r') as fp:
  line = fp.readline()
  line = line.strip()
  line_count = 0
  while True:
    ...
    line = line.replace('  ',' ')
    kkvv = line.split(' ')
    if 'train' in line and 'END' not in line:
      # 训练阶段日志
    ...
    if len(kkvv) != 21 and 'valid' in line and 'END epoch:' in line:
      # ceshi
      for mi, mc in enumerate(kkvv):
        if mc.startswith('top1_avg'):
          top_1.append((float)(kkvv[mi + 1]))

        if mc.startswith('top5_avg'):
...
loss = [t for ti,t in enumerate(loss) if ti % 10 == 1]
learning_rate = [t for ti,t in enumerate(learning_rate) if ti % 10 == 1]
